...
{
  Board board;
  board__load(&board, moves, size);
  return board;
}

void analyze__ongoing_turn_p1(void)
{
  Board board = board_stub__ongoing_turn_p1();

  GameStatusAnalyzer_Result result = game_status_analyzer__analyze(&board);
...
{
  Board board = board_stub__ongoing_turn_p2();
...
{
  static char MOVES[][2] = {A1, A3, B2, A2, C3};
  Board board = CREATE_GAME(MOVES);
...
{
  static char MOVES[][2] = {B1, A1, A3, B2, A2, C3};
  Board board = CREATE_GAME(MOVES);
